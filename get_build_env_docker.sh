...
docker run --network=host --gpus all --privileged --user $(id -u):$(id -g) -it -e DISPLAY=$DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix -v /build:/build -v /dev/bus/usb:/dev/bus/usb bsp-perf-build-env-x86:v0.0.1 /bin/bash

docker exec -it <container-id-or-name> /bin/bash

# run base docker image(if required to update base image for enviroment modification)
docker run --network=host --gpus all --privileged  -it -e DISPLAY=$DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix -v /build:/build -v /dev/bus/usb:/dev/bus/usb zczjx/bsp-perf-build-env-x86-base:latest /bin/bash
